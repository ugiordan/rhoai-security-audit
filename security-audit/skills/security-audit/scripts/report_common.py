...
_RE_FINDING_SPLIT = re.compile(r'\n(?=(?:Finding ID:|###?\s+(?:SEC|PERF|QUAL|CORR|ARCH|FINDING)-\d+))')
_RE_FINDING_ID = re.compile(r'(?:Finding ID:\s*|###?\s+)((?:SEC|PERF|QUAL|CORR|ARCH|FINDING)-\d+)')
_RE_SEVERITY = re.compile(r'Severity:\s*(\w+)', re.IGNORECASE)
...
_RE_LINE = re.compile(r'Lines?:\s*(\d+)')
_RE_EVIDENCE = re.compile(r'Evidence:\s*(.+?)(?=\n(?:Impact|Recommended|Finding ID:|\Z))', re.DOTALL)
_RE_FIX = re.compile(r'Recommended fix:\s*(.+?)(?=\n(?:Finding ID:|\Z))', re.DOTALL)
_RE_SNIPPET = re.compile(r'```[a-z]*\n(.*?)```', re.DOTALL)
_RE_BRACKET_SEV = re.compile(r'\n(?=##[#]?\s+\[(?:CRITICAL|HIGH|MEDIUM|LOW|INFO)\])')
_RE_BRACKET_HEADING = re.compile(r'##[#]?\s+\[(CRITICAL|HIGH|MEDIUM|LOW|INFO)\]\s+(.+?)(?:\n|$)')
_RE_NUMBERED_HEADING = re.compile(r'\n(?=#{3,4}\s+\d+\.)')
_SEV_MAP = {"critical": "critical", "important": "high", "high": "high", "medium": "medium", "minor": "low"}
